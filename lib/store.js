...
export function getSettings() {
  if (typeof window === 'undefined') return { theme: 'light', groqApiKey: process.env.NEXT_PUBLIC_GROQ_API_KEY || '', adminBackupCode: DEFAULT_ADMIN_BACKUP_CODE };
  initStore();
  const settings = JSON.parse(localStorage.getItem('ytkp_settings') || '{}');
  
  // FORCE OVERRIDE: If environment variable exists, ALWAYS use it over an empty/invalid localStorage value
  ...
  if (envKey && (!settings.groqApiKey || settings.groqApiKey.length < 10)) {
    ...
    // Auto-save the found env key back to localStorage so the UI updates
